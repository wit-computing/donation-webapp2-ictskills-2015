...
/**
 * Use ajax call to get users and their geolocations geoObj[0] is descripion. geoObj[1] is latitude geoObj[2] is longitude
 */
...
/**
 * data into an array, format is 'firstName, xx.xxxx, yy.yyyyy' -> (firstName, lat, lng) then invoke 'fitBounds' to render the markers, centre map and create infoWindow to display firstName
 */
...
/**
 * Convert the latlng string to individual numbers and thence to a google.maps.LatLng object
 */
...
/**
 * registers click listener to capture lat,lng clicked point data stored in array (pos[])
 */
function start () {
...
/**
 * Stop drawing the sequence of polylines Update listeners Invoke drawPolygon method
 */
function stop () {
...
/**
 * create and render a polyline on map attaches beginning to end previous polyline if such exists
 */
function polyline ( prevIndex, index ) {
	var coords = [
			new google.maps.LatLng( pos[ prevIndex ].lat(), pos[ prevIndex ].lng() ), new google.maps.LatLng( pos[ index ].lat(), pos[ index ].lng() )
	];
